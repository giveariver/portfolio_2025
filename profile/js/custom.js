    ...
    $(window).on("scroll",function(){	 //메뉴 활성화
	   ...
	   */
		...
		$("section").on("mousewheel",function(event, delta){    
			    var ind = $(this).index(); // 0 1 2 3 4
				var cnt = $("section").size()-1; //4
		        //console.log(cnt);
			    console.log(delta);
				//위로휠(+1) , 아래로휠(-1)
            //마우스 휠을 올렸을때	
...
function startIntro() {
		...
		setTimeout(function(){
			$(".ani01 .txt1").animate({opacity: 1}, 500, function(){	//주 나타나기
				setTimeout(function(){	
					$(".ani01 .txt1 span").css({width: "240px"});	//주도적인 생각
					$(".ani01 .txt2").animate({opacity:1}, 500, function(){	//호 나타나기
						setTimeout(function(){
							$(".ani01 .txt2 span").css({width: "280px"});	//호감가는 디자인
							setTimeout(function(){
								$(".ani01 .txt1 span, .ani01 .txt2 span").animate({opacity: 0}, 500);
								setTimeout(function(){
									$(".ani01 div").css({top: "50%", left: "51.5%"});
									setTimeout(function(){
										$(".intro.ani02").animate({opacity:1}, 700);
											setTimeout(function(){
												$(".intro.ani01, .intro.ani02").animate({opacity:0}, 700, function(){
